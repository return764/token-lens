...
TokenLens is a macOS menu bar app that monitors your LLM token consumption — automatically and locally. No proxy, no MITM, no network interception. It reads the local JSONL session logs produced by AI coding agents and shows you cost and token stats at a glance.
...
- **Auto-scanning** — reads Codex, Claude Code, and pi JSONL logs on startup
...
All three are always enabled. Missing directories are retried when the watcher detects them later.
...
TokenLens 是一款 macOS 菜单栏应用，自动监控本地 AI Coding Agent 的 LLM Token 用量。无需代理、无需 HTTPS 解密、无需网络拦截。它直接读取 agent 生成的 JSONL 会话日志，在菜单栏和设置窗口中展示费用与 Token 统计。

### 核心功能

- **菜单栏显示** — 展示今日/本月/全部的费用或 Token 数
- **实时 Token 动画** — 新用量到达时，菜单栏动态显示 input/output tokens
- **自动扫描** — 启动时自动读取 Codex、Claude Code、pi 的历史 JSONL 记录
...
三个 source 始终启用，目录不存在时静默跳过，后续出现后后台自动补扫。
