...
TokenLens 是一个 macOS 本地优先的 LLM token 用量监控应用。当前实现自动读取 Codex、Claude Code、pi 的本地 JSONL 记录，提取 token usage，写入 SQLite（`token_usages` 表），并在菜单栏和 Settings 展示消耗。
...
```text
Codex / Claude Code / pi 本地记录文件 (~/.codex, ~/.claude, ~/.pi)
  ↓
LocalUsageAdapter（per-source JSONL parser）
  ↓
LocalSourcesBackgroundService
  ...
  └─ 周期 reconcile 兜底
  ↓
LocalJSONLIncrementalReader（新增完整行 + byte offset checkpoint）
  ↓
...
| `local_scan_files` | 文件级增量 checkpoint（read_offset + parse_context_json） |
...
三个 source 始终启用。目录不存在时跳过，watcher 会周期重试。
...
| `AppStateTests` | 6 | refresh、菜单栏显示、设置持久化 |
...
| `LocalUsageScannerTests` | 4 | 默认 adapters、catch-up、not_found、parse_error 隔离 |
| `LocalJSONLIncrementalReaderTests` | 9 | offset、半行、空文件、truncate |
| `FileSystemEventWatcherTests` | 2 | JSONL path dedupe / directory expansion |
